...
class UserData {
    ...
    static func loadData(){
        ...
        for obj in data {
            let event = Event(obj)
            if(!allEventsContains(event)){
                appendToAllEvents(event)
                if(!selectedEventsContains(event) && added.contains(event.pk)){
